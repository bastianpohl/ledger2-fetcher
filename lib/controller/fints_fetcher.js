...
const bank = {
    ...
    name: process.env.FNTS_ACCOUNT,
    pin: process.env.FNTS_PIN,
    blz: process.env.FNTS_BLZ
}
const startDate = new Date("2019-08-01T12:00:00Z");
        ...
        try {
            ...
            for (let i = 0; i < fints.accounts.length; i++) {
                ...
                if (check.length === 0) {
                    ...
                    result = (check[0].value !== balance.value) ? await balance.update() : undefined
                }
                ...
                if (result) { // write transactions to the db
                    ...
                    let docs_result = await Promise.all(docs.map(async doc => {
                      return await doc.create()
                    }))
                    console.log(docs_result)
                }
            }
        } catch (error) {
            console.error(error);
